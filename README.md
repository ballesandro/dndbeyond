...
3. Run `docker run -d -p 8080:80 --name mydnd dndbeyond`
...
The second part is that you can find Swagger documentation at [https://localhost:8080/api](https://localhost:8080/api)

The third (bonus) part is an implementation of GraphQL. You can play around with this at [https://localhost:8080/api](https://localhost:8080/api). Some sample queries and mutations can be found in `DnDBeyond.sample_graphql.txt`. Please be sure to include the query variables found in `DnDBeyond.sample_graphql_variables.txt`. It can work in conjunction with the standard REST API as well.
